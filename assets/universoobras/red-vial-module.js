...
window.rvApplyStyle = function() {
    ...
    const style = {
        ...
        sources: {
            "protomaps": {
                type: "vector",
                url: "pmtiles://../data/tacna.pmtiles?v=2", // Obliga al servidor a no usar caché corrupta
    ...
    if (toggles['parks']) style.layers.push({ id: "parks", type: "fill", source: "protomaps", "source-layer": "landuse", paint: { "fill-color": ["match", ["get", "pmap:kind"], ["medical", "hospital"], t.amenity_med, ["education", "school", "university", "college"], t.amenity_edu, ["park", "pitch", "cemetery", "stadium", "wood", "grass", "forest"], t.parks, "rgba(0,0,0,0)"] } });
    ...
    if (toggles['roads']) {
        style.layers.push({ id: "roads-casing", type: "line", source: "protomaps", "source-layer": "roads", paint: { "line-color": ["match", ["get", "pmap:kind"], ["motorway", "trunk", "primary", "secondary"], t.road_main_case, t.road_minor_case], "line-width": ["interpolate", ["linear"], ["zoom"], 12, ["match", ["get", "pmap:kind"], ["motorway", "trunk", "primary", "secondary"], 4, 1.5], 16, ["match", ["get", "pmap:kind"], ["motorway", "trunk", "primary", "secondary"], 14, 6]] } });
        style.layers.push({ id: "roads", type: "line", source: "protomaps", "source-layer": "roads", paint: { "line-color": ["match", ["get", "pmap:kind"], ["motorway", "trunk", "primary", "secondary"], t.road_main, t.road_minor], "line-width": ["interpolate", ["linear"], ["zoom"], 12, ["match", ["get", "pmap:kind"], ["motorway", "trunk", "primary", "secondary"], 2.5, 0.5], 16, ["match", ["get", "pmap:kind"], ["motorway", "trunk", "primary", "secondary"], 10, 4]] } });
    }
    ...
    if (toggles['places-text']) {
        ...
        if (toggles['roads']) style.layers.push({ id: "roads-text", type: "symbol", source: "protomaps", "source-layer": "roads", filter: ["all", ["has", "name"], ["in", "pmap:kind", "motorway", "trunk", "primary", "secondary"]], layout: { "text-field": ["get", "name"], "symbol-placement": "line", "text-font": ["Noto Sans Regular"], "text-size": ["interpolate", ["linear"], ["zoom"], 12, 10, 16, 13], "text-max-angle": 30, "text-pitch-alignment": "viewport" }, paint: { "text-color": t.road_text, "text-halo-color": t.road_minor, "text-halo-width": 2 } });
    }
    if (toggles['pois-text']) style.layers.push({ id: "pois-text", type: "symbol", source: "protomaps", "source-layer": "pois", filter: ["in", "pmap:kind", "medical", "hospital", "police", "education", "school", "university", "college", "transit", "bus_station", "government", "town_hall", "stadium", "market"], layout: { "text-field": ["concat", ["match", ["get", "pmap:kind"], ["medical", "hospital"], "🏥 ", ["police"], "🚓 ", ["education", "school", "university", "college"], "🏫 ", ["transit", "bus_station"], "🚌 ", ["government", "town_hall", "public"], "🏛️ ", ["stadium"], "⚽ ", ["market"], "🛒 ", "📍 "], ["get", "name"]], "text-font": ["Noto Sans Regular"], "text-size": ["interpolate", ["linear"], ["zoom"], 14, 11, 16, 13], "text-anchor": "bottom", "text-offset": [0, 0.5] }, paint: { "text-color": t.poi, "text-halo-color": t.bg, "text-halo-width": 1.5 } });
